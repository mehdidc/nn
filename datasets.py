...
def data_file(data_type, f):
    return os.path.join(data_path, data_type, f)
...
def cifar_example(nb=1, nb_features=4096):
    if nb==7:
        ...
        for n in (1, 2, 3, 4, 5, 6):
            data, labels = np.load(data_file("cifar", "cifar%d_%d.npy" % (n, nb_features)))
...
def mnist_mimic_example():
    data, targets = np.load(data_file("mnist", "mnist_new_pca.npy"))
    data = np.array(list(data))
...
def mnist_example(which='train'):

    data, targets = np.load(data_file("mnist", "%s.npy" % (which,)  ))
    data = np.array(list(data))
    data = data.reshape( (data.shape[0], np.prod(data.shape[1:])))
...
def breast_example():
    dataset = np.loadtxt(data_file("uci", "breast.txt"), delimiter=",")
    x, y = dataset[:, 0:-1], dataset[:, -1]
    y = np.array([[yi] for yi in y])
    return x, y

def plants_example():
    dataset = np.loadtxt(data_file("plants", "data_Mar_64.txt"), delimiter=",", dtype="string")
...
def taxonomy_example(name):
    fd = open(  data_file(taxonomy, name), "r")
    x = []
    y = []
